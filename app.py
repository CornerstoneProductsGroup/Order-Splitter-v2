...
def pixmap_nonwhite_ratio(pix: fitz.Pixmap) -> float:
    if pix.alpha:
...
def auto_content_rect(page: fitz.Page, margin: float = 6.0) -> fitz.Rect | None:
    """Return the bounding rect of all vector content (text + drawings) on the page.

    Auto-detects where the label actually is so no manual coordinate
...
def render_sos_clip_pixmap(src_page: fitz.Page, region: dict) -> tuple[fitz.Pixmap, fitz.Rect]:
    ...
    clip = auto_content_rect(src_page)
    if clip is None:
        clip = region_to_rotated_rect(src_page, region)
    try:
        pix = src_page.get_pixmap(matrix=fitz.Matrix(2, 2), clip=clip, alpha=False)
        if pixmap_nonwhite_ratio(pix) > 0.003:
            return pix, clip
    except Exception:
        pass
